...
```{r}
correct_ICh<-unlist(mapply(function(i,j){if ((i-2*sqrt((1-3/8)*j^2/3))<yu & yu<(i+2*sqrt((1-3/8)*j^2/3))) {1}else{0}},ech.all$ybar,ech.all$s))
...
```{r}
correct_ICi<-unlist(mapply(function(i,j){if ((i-3*sqrt((1-3/8)*j^2/3))<yu & yu<(i+3*sqrt((1-3/8)*j^2/3))) {1}else{0}},ech.all$ybar,ech.all$s))
...
correct_ICj<-unlist(lapply(ech.all$ybar,function(i){if ((i-2*sigma_y_bar)<yu & yu<(i+2*sigma_y_bar)) {1}else{0}}))
...
sigma<-sum(unlist(mapply(function(i,j){(i-y_u)^2*j},y,e)))
var_y_th<-sigma^2/length(y)
...
curve(dnorm(x,y_u,sqrt(var_y_emp)),add=TRUE)
```

## d - Probabilité que $\bar{y}$ soit éloigné de plus de 1.96 écart-type de la moyenne

```{r}
out<-lapply(y_bar,function(i){if (mean(y_bar)-1.96*var_y_emp<i & i<mean(y_bar)+1.96*var_y_emp) {0}else{1}})
```

La probabilité que $\bar{y}$ soit éloigné de plus de 1.96 écart-type de la moyenne est de `r round(sum(unlist(out))/length(out)*100)`%.
...
## g - Autre intervalld de confiance à 95%
